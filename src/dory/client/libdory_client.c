/* <dory/client/libdory_client.c>
   ...
   Dory client C library implementation.
 */
...
#include <dory/client/build_id.h>
#include <dory/input_dg/any_partition/v0/v0_write_msg.h>
#include <dory/input_dg/partition_key/v0/v0_write_msg.h>

const char EXPORT_SYM *dory_get_build_id() {
  return dory_client_lib_build_id;
}

int EXPORT_SYM dory_find_any_partition_msg_size(size_t topic_size,
...
int EXPORT_SYM dory_find_partition_key_msg_size(size_t topic_size,
    size_t key_size, size_t value_size, size_t *out_size) {
  assert(out_size);
...
int EXPORT_SYM dory_write_any_partition_msg(void *out_buf,
  ...
  size_t dg_size = 0;
  int ret = dory_find_any_partition_msg_size(topic_len, key_size, value_size,
      &dg_size);
...
int EXPORT_SYM dory_write_partition_key_msg(void *out_buf,
  ...
  size_t topic_len = strlen(topic);
  size_t dg_size = 0;
  int ret = dory_find_partition_key_msg_size(topic_len, key_size, value_size,
      &dg_size);
...
void EXPORT_SYM dory_client_socket_init(
    dory_client_socket_t *client_socket) {
...
int EXPORT_SYM dory_client_socket_bind(dory_client_socket_t *client_socket,
    const char *server_path) {
  assert(client_socket);
  assert(server_path);

  if (client_socket->sock_fd >= 0) {
    return DORY_CLIENT_SOCK_IS_OPENED;
  }
...
int EXPORT_SYM dory_client_socket_send(
    const dory_client_socket_t *client_socket, const void *msg,
    size_t msg_size) {
  assert(client_socket);
  assert(msg);
  assert(msg_size);
  ...
  return (ret < 0) ? errno : DORY_OK;
}

void EXPORT_SYM dory_client_socket_close(
    dory_client_socket_t *client_socket) {
  assert(client_socket);
