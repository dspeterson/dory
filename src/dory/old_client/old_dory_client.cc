/* <dory/old_client/old_dory_client.cc>
   ...
   Old Dory client.  This is going away soon.
 */
...
static void ParseArgs(int argc, char *argv[], TConfig &config) {
  ...
  try {
    ...
    ValueArg<decltype(config.SocketPath)> arg_socket_path("", "socket_path",
        ...
        "Dory.", true, config.SocketPath, "PATH");
    ...
    SwitchArg arg_use_old_input_format("", "use_old_input_format", "Send "
        "messages to dory using legacy input format.", cmd,
...
bool CreateDg(std::vector<uint8_t> &buf, const TConfig &cfg,
  ...
  if (cfg.UsePartitionKey) {
    ...
    if (dory_write_partition_key_msg(&buf[0], buf.size(), cfg.PartitionKey,
    ...
    if (dory_write_any_partition_msg(&buf[0], buf.size(), cfg.Topic.c_str(),
...
int old_dory_client_main(int argc, char **argv) {
  ...
  switch (ret) {
    ...
    case DORY_SERVER_SOCK_PATH_TOO_LONG:
      ...
      return EXIT_FAILURE;
    default:
      std::cerr << "Unexpected result from Dory socket initialization: "
          << ret << std::endl;
      return EXIT_FAILURE;
  }
  ...
  for (size_t i = 1; i <= cfg->Count; ++i) {
    ...
    if (ret) {
      std::cerr << "Error sending to Dory: " << std::strerror(ret)
          << std::endl;
      return EXIT_FAILURE;
    }
...
int main(int argc, char **argv) {
  ...
  try {
    ret = old_dory_client_main(argc, argv);
