/* <dory/input_dg/any_partition/v0/v0_write_msg.h>
   ...
   Functions for creating AnyPartition datagrams to write to Dory's input
   socket.
 */
...
/* See <dory/client/status_codes.h> for definitions of returned status codes.
 */

/* Return DORY_OK if datagram can be created with the given field sizes in
   bytes.  Otherwise return DORY_TOPIC_TOO_LARGE or DORY_MSG_TOO_LARGE. */
...
/* Compute size of datagram with field sizes given by 'topic_size', 'key_size',
   and 'value_size'.  On success, DORY_OK will be returned, and *result will
   contain the computed size.  All sizes are in bytes.  On error,
   DORY_TOPIC_TOO_LARGE or DORY_MSG_TOO_LARGE will be returned. */
int input_dg_any_p_v0_compute_msg_size(size_t *result, size_t topic_size,
    size_t key_size, size_t value_size);

/* Write datagram into 'result_buf'.  It is assumed that 'result_buf' has
enough space for entire datagram (see input_dg_any_p_v0_compute_msg_size()). */
