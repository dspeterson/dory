...
# -----------------------------------------------------------------------------
#
# This script requests counter reports from dory and analyzes the counters.
...
#        of getting counters from dory and saving them to a counter file, we
...
#
#    -b HOST
#    --dory_host HOST: (required unless running in manual mode)
#        The host running dory that we should connect to for counter data.
#
#    -p PORT
#    --dory_status_port PORT: (optional, default = 9090)
#        The port to connect to when asking dory for counter data.
#
#    -u USER
...
###############################################################################
# Send a "get counters" HTTP request to dory, parse the response, and return
...
# currently running instance of dory.  Return a counter report containing the
...
def GetLastCounterReport(current_report, work_path, old_counter_file_times):
    ...
    # invocation of dory (i.e. dory restarted).  Therefore the data is not
    # applicable.
...
# instance of dory.  The most recent file that was created at least a given
...
def GetOldCounterReport(current_report, work_path, old_counter_file_times,
    ...
    # The old counter file contains data for a different invocation of dory
    # (i.e. dory restarted).  Therefore the data is not applicable.
...
# directly from dory) and the previous counter report (obtained from a file).
...
def AnalyzeDeltas(deltas):
    ...
    # dory's HTTP status monitoring mechanism, which means that the integrity
...
class TProgramOptions(object):
    ...
    def __init__(self, manual, work_dir, interval, history, nagios_server,
                 ...
                 msg_count_critical_threshold, dory_host, dory_status_port,
                 nagios_user):
        self.Manual = manual
...
def ParseArgs(args):
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'm:d:i:H:s:w:c:W:C:b:p:u:',
                ['manual', 'work_dir=', 'interval=', 'history=',
                 ...
                 'dory_host=', 'dory_status_port=', 'nagios_user='])
    ...
    if (opt_dory_host == '') and (opt_manual == -1):
        Die(EC_UNKNOWN, '-b or --dory_host option must be specified')
    ...
    return TProgramOptions(opt_manual, opt_work_dir, opt_interval,
                           ...
                           opt_msg_count_critical_threshold, opt_dory_host,
                           opt_dory_status_port, opt_nagios_user)
###############################################################################
...
def main():
    ...
    if RunningInManualMode():
        ...
        counter_report = GetCounters('http://' + Opts.DoryHost + ':' + \
                str(Opts.DoryStatusPort) + '/counters/json')
