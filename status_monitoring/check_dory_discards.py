...
# -----------------------------------------------------------------------------
#
# This script requests a discard report from dory, analyzes it, stores it in
...
#
#    -b HOST
#    --dory_host HOST: (required)
#        The host running dory that we should connect to for discard data.
...
#
#    -p PORT
#    --dory_status_port PORT: (optional, default = 9090)
#        The port to connect to when asking dory for discard data.
#
#    -t TESTFILE
#    --testfile TESTFILE: (optional)
#        This is for testing.  If specified, the discard report is obtained
#        from file TESTFILE rather than connecting to dory.  When using this
#        option, you must still specify the host running dory (-b option),
...
###############################################################################
# Send a "get discards" HTTP request to dory and return the response as a
# string.
###############################################################################
def GetDiscardResponse(url):
    try:
        ...
        # Treat this as Critical, since it indicates that dory is probably not
        # running.
...
# If we got the response directly from Dory, then 'host' and 'port' will
# provide the canonicalized hostname where Dory is running, and the port Dory
...
# DORY_DATA_QUALITY_REPORT table.  On success, return the integer value from
...
def InsertReportRow(cur, report):
    # Add report row to DORY_DATA_QUALITY_REPORT table.
    ...
    sql = 'insert into DORY_DATA_QUALITY_REPORT ' + \
          '    (HOST, DORY_PORT, DORY_REPORT_ID, REPORT_START, ' + \
    ...
    sql = 'select ID from DORY_DATA_QUALITY_REPORT where HOST = :host ' + \
          ' and DORY_PORT = :port and REPORT_START = ' + \
...
# where ID is a foreign key into the DORY_DATA_QUALITY_REPORT table.  Input
...
###############################################################################
# Insert a set of rows into table DORY_RATE_LIMIT_DISCARD.  Input parameter
# 'cur' provides a database cursor, and 'report_id' provides the discard report
# ID, which is a foreign key into table DORY_DATA_QUALITY_REPORT.
...
# on discards due to Dory's message rate limiting mechanism.
###############################################################################
def InsertRateLimitDiscardRows(cur, report_id, topic_max_size,
    ...
    sql = 'insert into DORY_RATE_LIMIT_DISCARD(ID, TOPIC, MSG_COUNT) ' + \
            'values(:1, :2, :3)'
    cur.executemany(sql, rows)
###############################################################################

###############################################################################
# Insert a set of rows into either table DORY_DISCARD_TOPIC or
# DORY_POSSIBLE_DUP_TOPIC, depending on the text blurb in input parameter
...
# DORY_DATA_QUALITY_REPORT.  Input parameter 'topic_max_size' specifies the
...
def DoPersistDiscardReportImpl(con, cur, report):
    ...
    # Add rows to DORY_TOO_LONG_MSG table.
    InsertStringRows(cur, report_id, 'DORY_TOO_LONG_MSG(ID, MSG)', 4000,
                     report['recent_too_long_msg'])

    if report['unsupported_msg_version']:
        # Add rows to DORY_UNSUPP_VERSION_MSG table.
        ...
        sql = 'insert into ' + \
              'DORY_UNSUPP_VERSION_MSG(ID, MSG_VERSION, MSG_COUNT) ' + \
          'values (:1, :2, :3)'
        cur.executemany(sql, rows)

    # Add rows to DORY_BAD_TOPIC table.
    InsertStringRows(cur, report_id, 'DORY_BAD_TOPIC(ID, TOPIC)', 100,
                     report['recent_bad_topic'])

    # Add rows to DORY_RATE_LIMIT_DISCARD table.
    InsertRateLimitDiscardRows(cur, report_id, 100,
            report['rate_limit_discard'])

    # Add rows to DORY_DISCARD_TOPIC table.
    InsertTopicErrorInfoRows(cur, report_id,
            'DORY_DISCARD_TOPIC(ID, TOPIC, TIMESTAMP_1, TIMESTAMP_2, ' +
            'DISCARD_COUNT)', 100, report['discard_topic'])

    # Add rows to DORY_POSSIBLE_DUP_TOPIC table.
    InsertTopicErrorInfoRows(cur, report_id,
            'DORY_POSSIBLE_DUP_TOPIC(ID, TOPIC, TIMESTAMP_1, TIMESTAMP_2, ' +
...
#     DoryHost: The host running dory that we should connect to for counter
#         data.
#     DoryStatusPort: The port to connect to when asking dory for counter
#         data.
...
class TProgramOptions(object):
    ...
    def __init__(self, verbose, work_dir, nagios_server, max_report_age,
                 nagios_user, dory_host, dory_status_port, testfile, dbhost,
        ...
        self.DoryHost = dory_host
        self.DoryStatusPort = dory_status_port
...
def ParseArgs(args):
    try:
        opts, args = getopt.getopt(sys.argv[1:],
                ...
                ['verbose', 'work_dir=', 'nagios_server=', 'max_report_age=',
                 'nagios_user=', 'dory_host=', 'dory_status_port=',
    ...
    for o, a in opts:
        if o in ('-v', '--verbose'):
            ...
            opt_nagios_user = a
        elif o in ('-b', '--dory_host'):
            opt_dory_host = a
        elif o in ('-p', '--dory_status_port'):
            try:
                opt_dory_status_port = int(a)
            except ValueError:
                Die(EC_UNKNOWN, 'The ' + o + ' option requires an integer')

            if (opt_dory_status_port < 1) or (opt_dory_status_port > 65535):
                Die(EC_UNKNOWN,
                    'The ' + o +
    ...
    return TProgramOptions(opt_verbose, opt_work_dir, opt_nagios_server,
                           opt_max_report_age, opt_nagios_user, opt_dory_host,
                           opt_dory_status_port, opt_testfile, opt_dbhost,
...
def main():
    ...
    if Opts.Testfile == '':
        # Get discard info from dory.
        json_response = GetDiscardResponse('http://' + host + ':' + \
                str(Opts.DoryStatusPort) + '/discards/json')
    else:
        json_response = ReadFileContents(Opts.Testfile)

    response = ParseDiscardResponse(json_response, host, Opts.DoryStatusPort)
